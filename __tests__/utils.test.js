const { articleCommentCount } = require('../app/utils');
const {
...
describe('articleCommentCount function', () => {
	test('purity test', () => {
		const input = [
			{ article_id: 9 },
			{ article_id: 1 },
		...
		expect(articleCommentCount(input)).not.toBe(input);
	});
	test('mutation test', () => {
		const input = [
			{ article_id: 9 },
